...
Für den ersten Windows-Port wurden Visual Studio 2022/MSVC, das mitgelieferte CMake und Qt 6.9.3 mit Multimedia verwendet. Debug- und Release-Build kompilieren nativ; alle 23 Tests laufen unter Windows erfolgreich. Die Named-Pipe-Kompatibilität wurde zusätzlich gegen den laufenden Windows-Boltsnap-Daemon geprüft. Ein portables Qt-Deployment wird unter `dist/eddy` erzeugt. Zusätzlich erzeugen `packaging/windows/build-msi.ps1` eine ICE-validierte x64-MSI und `packaging/windows/build-nsis.ps1` einen funktionsgleichen x64-NSIS-Installer mit Qt- und MSVC-Laufzeit, Startmenüeintrag sowie Windows-Dateizuordnungen. Externe `ffmpeg`-/`ffprobe`-Programme und Tesseract sind noch nicht paketiert und bleiben der nächste Funktionsblock für vollständigen Videoexport und OCR.
